...
| `analysis/arxivtopics/global_phasor.py` | **the deployed model**: y = \|b_j + A_j·Σ aᵢe^{i(θᵢ−p_ij)}\|² — one global arrow spectrum; each field a level, a gain and seven phases; squared-envelope expansion (transits + aspects) fitted in closed form, level and gain by exact cubic roots — 0.837 twelve-origin mean / 0.769 headline; the all-per-field record (0.875 / 0.799) prices one shared spectrum at 0.038 ||
