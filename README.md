...
| `analysis/arxivtopics/global_phasor.py` | **the deployed model**: one global receiver (level + seven arrow lengths shared by all fields), each field characterised by its seven phases alone; the squared envelope expands into transit + aspect terms, fitted in closed form — 0.81 twelve-origin mean / 0.73 headline, vs the per-field record's 0.88 / 0.80 |
