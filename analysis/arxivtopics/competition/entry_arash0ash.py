...
CFGS = [dict(lam=l, lr=r, steps=s) for l in (0.01, 0.03, 0.1) for r in (0.01, 0.03) for s in (3000, 8000)]
scores = []
for ci, cfg in enumerate(CFGS):
    if left() < 0.35 * BUDGET_H * 3600: print("budget guard: stopping config sweep"); break
    preds = [fit_phasor(INNER, cfg["lam"], cfg["lr"], cfg["steps"], seed)[:, INNER:INNER + 30] for seed in (7, 11)]
...
SEEDS = (7, 11, 23, 42, 5, 31)
finals = []
for sd in SEEDS:
    if left() < 900: break
    finals.append(fit_phasor(nyr, best["lam"], best["lr"], best["steps"] * 2, sd))
    print(f"final seed {sd} done · {left()/3600:.1f}h left", flush=True)
if not finals:
...
write_submission(P[:, YI[1996]:YI[1996] + 30],
                 meta={"family": "deep per-field phasor (Adam, seed ensemble)", "cfg": best, "seeds": len(finals)})
